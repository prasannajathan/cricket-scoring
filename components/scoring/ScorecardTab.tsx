...
const FallOfWickets: React.FC<FallOfWicketsProps> = ({ innings, state }) => {
  ...
  const dismissals = innings.deliveries
    ...
    .map((del, idx) => {
      ...
      let overNumber = Math.floor(legalBalls.length / 6);
      let ballNumber = legalBalls.length % 6;

      // Adjust to show sixth ball as "X.6" instead of "X+1.0"
      if (ballNumber === 0 && legalBalls.length > 0) {
        overNumber -= 1;
        ballNumber = 6;
      }
      ...
      return `${runsAtDismissal}-${idx + 1} (${outBatter.name}, ${overNumber}.${ballNumber})`;
    });
  ...
  return (
    <View style={styles.scorecardSection}>
      <Text style={styles.sectionHeader}>
        ...
        <Text style={styles.headerText}> {battingTeam.teamName} Innings </Text>
        <Text style={styles.headerScore}>{innings.totalRuns}/{innings.wickets}({innings.completedOvers}.{innings.ballInCurrentOver} Ov)</Text>
      </Text>
      ...
      {!isLoading && (
        <>
          ...
          <BowlingScorecard
            ...
            bowlingTeam={firstInningsBowlingTeam}
          />
          <PartnershipHistory innings={firstInnings} battingTeam={firstInningsBattingTeam} />
        </>
      )}
      ...
      {(secondInnings.totalRuns > 0 || secondInnings.isCompleted) && (
        <>
          ...
          <BowlingScorecard
            ...
            bowlingTeam={secondInningsBowlingTeam}
          />
          <PartnershipHistory innings={secondInnings} battingTeam={secondInningsBattingTeam} />
        </>
      )}
    </ScrollView>
  );
